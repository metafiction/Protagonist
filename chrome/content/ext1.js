...
var timekeeper = {
  ...
  update: function(){
    ...
    clearUserPref(MAIN_PREF + "pressI");
    clearUserPref(MAIN_PREF + "pressO");
    clearUserPref(MAIN_PREF + "pressP");
    clearUserPref(MAIN_PREF + "pressU");

    if(!isFile(this.path)){
      setBoolPref(MAIN_PREF + "pressI", false);

      if(isFile(this.target, ".tmp") && this.path != parentPath(this.target)){
        setBoolPref(MAIN_PREF + "pressO", false);
        setBoolPref(MAIN_PREF + "pressP", false);
      }
    }

    if(!hasFolder(this.path) && !hasFile(this.path) && this.path.match(/.tmp$/))
      setBoolPref(MAIN_PREF + "pressU", false);

    document.getElementById("pressU").setAttribute("disabled", !prefHasUserValue(MAIN_PREF + "pressU"));
    document.getElementById("pressI").setAttribute("disabled", !prefHasUserValue(MAIN_PREF + "pressI"));
    document.getElementById("pressO").setAttribute("disabled", !prefHasUserValue(MAIN_PREF + "pressO"));
    document.getElementById("pressP").setAttribute("disabled", !prefHasUserValue(MAIN_PREF + "pressP"));
  },
  ...
  unregister: function(){
    ...
    this._branch.clearUserPref(MAIN_PREF + "pressU");
    this._branch.clearUserPref(MAIN_PREF + "pressI");
    this._branch.clearUserPref(MAIN_PREF + "pressO");
    this._branch.clearUserPref(MAIN_PREF + "pressP");
  ...
  observe: function(aSubject, aTopic, aData){
    ...
    if(aData == MAIN_PREF + "pressEsc"){
      ...
      this.list.scroll(3);
    }else if(aData == MAIN_PREF + "pressI"){
      let tmp = localFile(this.path);

      try{
        tmp.appendRelativePath("protagonist.tmp");
        tmp.createUnique(Ci.nsIFile.NORMAL_FILE_TYPE, 0664);
        this.path = tmp.path;
        return;
      }catch(ex){
        alert("Error");
      }
    }else if(aData == MAIN_PREF + "pressP"){
      localFile(this.target).copyTo(localFile(this.path), leafName(this.target));
      setBoolPref(MAIN_PREF + "pressY", true);
      return;
    }else if(aData == MAIN_PREF + "pressO"){
      localFile(this.target).moveTo(localFile(this.path), leafName(this.target));
      setPrefLS(EXTRA_PREF + "path", parentPath(getPrefLS(EXTRA_PREF + "path")));
      setBoolPref(MAIN_PREF + "pressY", true);
      return;
    }else if(aData == MAIN_PREF + "pressU"){
      localFile(this.path).remove(true);
      setBoolPref(MAIN_PREF + "pressA", true);
      return;
